...
export function ExamApp() {
  ...
  const errorQuestions = useMemo(() => {
    ...
    for (const attempt of attempts) attempt.questionIds.forEach((id) => {
      ...
      const response = attempt.answers[id];
      if (question && response !== undefined && response !== (attempt.correctAnswers?.[id] ?? question.correctIndex)) ids.add(id);
    });
...
function Performance({ attempts }: { attempts: Attempt[] }) { const modules = new Map<string, { correct: number; answered: number }>(); attempts.forEach((attempt) => Object.entries(attempt.moduleStats).forEach(([name, stat]) => { const current = modules.get(name) ?? { correct: 0, answered: 0 }; const answered = stat.correct + (stat.wrong ?? Math.max(0, stat.total - stat.correct - (stat.unanswered ?? 0))); modules.set(name, { correct: current.correct + stat.correct, answered: current.answered + answered }); })); return <div className="content"><div className="page-heading"><p className="eyebrow">ESTATÍSTICAS</p><h1>Desempenho por módulo</h1><p>Somente questões respondidas entram no cálculo de erros por módulo.</p></div><section className="module-panel performance-panel">{modules.size ? [...modules].map(([module, stat]) => { const value = stat.answered ? Math.round((stat.correct / stat.answered) * 100) : 0; return <div className="performance-row" key={module}><div className="performance-score"><strong>{value}%</strong><span>{stat.correct}/{stat.answered} respondidas certas</span></div><div><h2>{module}</h2><div className="module-bar"><i style={{ width: `${value}%` }} /></div><p>{stat.answered === 0 ? "Ainda sem respostas" : value >= 75 ? "Ponto forte" : value >= 55 ? "Em evolução" : "Prioridade de revisão"}</p></div></div>; }) : <Empty text="As estatísticas serão calculadas após seu primeiro simulado." />}</section></div>; }
